...
import shap
together = False
if together:#plot together
    rf_explainer = shap.TreeExplainer(rf_classifier)

    rf_shap_values = rf_explainer.shap_values(X_test_scaled)

    shap_values_for_positive_class = rf_shap_values[1]

    # Feature names
    feature_names = X_train.columns.tolist() 

    # Check if the number of features matches the number of SHAP values
    assert shap_values_for_positive_class.shape[1] == len(feature_names), "Mismatch in the number of features and SHAP values"
    ...
    shap.force_plot(rf_explainer.expected_value[1], shap_values_for_positive_class[0], feature_names=feature_names)

    # Create a summary plot
    shap.summary_plot(shap_values_for_positive_class, X_test_scaled, feature_names=feature_names)
